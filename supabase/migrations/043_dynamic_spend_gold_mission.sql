-- ============================================
-- 043: "골드 사용하기" 미션 목표치를 뽑기레벨에 따라 동적으로 상향
-- 기존엔 10000골드 고정이라, 스킬/장비 뽑기를 많이 해서 골드를 대량으로 굴리는 유저에게
-- 미션이 너무 쉽게 완료돼버리는 문제가 있었음.
-- 스킬 뽑기레벨(total_skill_draws 기준)과 4개 장비 슬롯 뽑기레벨(equipment_gacha_progress 기준)의
-- 평균을 "종합 뽑기레벨"(1~20)로 보고, 그 레벨에 비례해서 10만 골드 단위로 계단식 상승시킴.
--   종합 뽑기레벨 1~3   -> 100,000골드
--   종합 뽑기레벨 4~8   -> 200,000골드
--   종합 뽑기레벨 9~12  -> 300,000골드
--   종합 뽑기레벨 13~17 -> 400,000골드
--   종합 뽑기레벨 18~20 -> 500,000골드 (최대)
-- ============================================
...
begin
  ...
  select coalesce(avg(least(20, 1 + draws / 1000)), 1) into v_equip_draw_level_avg
