-- ============================================
-- 156: 연승의 던전(155) 마일스톤 업적 3종 추가 - 신규 콘텐츠
-- 무한의 탑 업적(074/097 tower_10/30/100)과 동일한 설계 패턴을 그대로 따름 -
-- streak_dungeon_best.best_streak 기준으로 판정, 뱅킹 여부와 무관하게 "도달한
-- 최고 연승"으로 인정(155의 continue_streak_dungeon이 승리 시점마다 best를 갱신하므로,
-- 굳이 골드를 수령하지 않고 포기해도 업적 진행도는 그대로 남음).
--
-- check_achievement_eligibility/claim_all_achievements 둘 다 CASE 분기 및 키 배열
-- 추가만 있을 뿐 반환 컬럼 구성은 그대로라 DROP FUNCTION 불필요(153의 리팩터링
-- 의도 그대로: 판정 로직은 반드시 한 곳(check_achievement_eligibility)에만 두고
-- claim_achievement/claim_all_achievements가 재사용 - 이번에도 새 업적 키 하나
-- 추가에 CASE 분기 1곳 + 배열 1곳만 건드리면 되어 이중관리 위험이 낮음).
--
-- 신규 업적 3개(claimedAchievementKeys/ACHIEVEMENT_CATALOG는 클라이언트에서 별도 추가):
--   streak_10 (4,000골드) / streak_25 (18,000골드) / streak_50 (70,000골드, 엔드게임 자랑거리)
-- ============================================
