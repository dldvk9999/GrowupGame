-- ============================================
-- 114: 누적 획득 골드(lifetime) 추적 + 재산 업적 3종 - 신규 콘텐츠
-- 기존 "재산" 랭킹/업적 관련 지표는 전부 profiles.gold(현재 보유액)라, 강화/합성/뽑기로
-- 다 써버리면 그동안 열심히 번 기록이 사라지는 느낌이었음. add_gold는 골드가 지급되는
-- 거의 모든 경로(자동사냥/스테이지/던전/업적/출석/PvP/월드보스/우편 등)가 공통으로 거치는
-- 단일 지점이라, 여기에 "역대 총 획득량"을 같이 누적하면 한 곳만 고쳐도 전부 반영됨.
--
-- 주의: 이 마이그레이션 이전에 지급된 골드는 소급 집계되지 않음(0부터 시작) - last_login_at(102)
-- 등 다른 신규 컬럼들과 동일하게, 지금 이 시점부터의 누적만 추적.
--
-- add_gold는 반환타입(void) 그대로, claim_achievement도 반환타입(integer) 그대로라
-- 둘 다 DROP FUNCTION 불필요.
-- ============================================
...
-- claim_achievement에 재산(누적 획득) 업적 3종 CASE만 추가 (113 기준 재정의)
