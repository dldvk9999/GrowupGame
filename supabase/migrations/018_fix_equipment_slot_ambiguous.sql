-- ============================================
-- 018: draw_equipment / draw_equipment_batch의
-- "column reference slot is ambiguous" 버그 수정
-- (RETURNS TABLE의 출력 컬럼명 slot이 함수 내부에서 암시적 변수로도 잡혀서,
--  UPDATE ... WHERE slot = ... 처럼 테이블 별칭 없이 쓴 부분이 모호해지던 문제.
--  테이블 별칭을 붙여서 명확히 구분함 - 007에서 skill_key를 고쳤던 것과 동일 패턴)
-- Supabase SQL Editor에 순서대로 실행 (001~017 먼저 적용되어 있어야 함)
-- ============================================
