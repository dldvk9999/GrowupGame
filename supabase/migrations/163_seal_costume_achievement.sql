-- ============================================
-- 163: 봉인의 상점(162) 세트 완성 업적 추가 - 신규 콘텐츠
-- 4종 전부 보유 시 업적 지급(장착 여부와 무관 - full_set_equipped와 달리 "구매"
-- 자체가 파편을 소비하는 행위라 이미 충분히 의미 있는 마일스톤이라고 판단, 착용까지
-- 강제하지 않음). 보상은 골드로만 지급, 파편 재화와는 여전히 분리(156/160과 동일 원칙).
--
...
begin
  ...
  case p_achievement_key
    ...
    when 'seal_costume_set' then
      select count(*) >= 4 into v_eligible from public.seal_costume_inventory where user_id = auth.uid();
      v_reward := 8000;
