-- ============================================
-- 176: PvP - 적합한 실유저 상대가 없을 때 항상 가상 상대를 만들던 것을 30% 확률로만
-- 만들고, 나머지 70%는 "적합한 상대가 없어요" 안내만 하고 전투 없이 종료 - 사용자 요청.
-- start_pvp_battle 재정의(반환타입 그대로, DROP 불필요) - 반환값에 result='no_opponent'
-- 라는 새 케이스가 추가됨(기존 'win'/'lose' 외). 이 경우 my_power만 의미 있고
-- opponent_power/reward는 0, opponent_name은 null.
-- ============================================
...
begin
  ...
  if v_found_opponent then
    ...
    if v_opp_power < v_my_power * 0.75 or v_opp_power > v_my_power * 1.25 then
      v_found_opponent := false; -- 전투력 격차가 크면 실유저 매칭 취소, 아래에서 가상 상대 여부를 다시 결정
    end if;
  end if;

  if not v_found_opponent then
    -- (신규, 사용자 요청) 예전엔 여기서 무조건 가상 상대를 만들었는데, 이제 30%만 가상
    -- 상대를 만들고 나머지 70%는 전투 없이 "적합한 상대가 없어요"만 안내하고 종료함.
    -- 이 경우 재도전 쿨다운(last_pvp_battle_at)은 갱신하지 않음 - 실제로 아무 일도
    -- 안 일어났으니 바로 다시 시도할 수 있어야 자연스러움.
    if random() >= 0.30 then
      return query select 'no_opponent'::text, null::text, false, v_my_power, 0, 0,
