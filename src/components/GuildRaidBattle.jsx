...
import { reportGuildRaidDamage } from '../lib/guildRaid';
...
// 월드보스보다 참여 인원이 적은(최대 30명) 길드 단위 레이드라 데미지/체력 규모를 축소.
// 공격 텀도 살짝 여유있게(0.8초) - 월드보스보다 소규모 인원이 오래 버텨야 하는 특성 반영.
const GUILD_RAID_NORMAL_DAMAGE = 6000;
const GUILD_RAID_SKILL_DAMAGE = 13000;
const GUILD_RAID_ATTACK_INTERVAL = 800;
const GUILD_RAID_SKILL_EVERY_N_ATTACKS = 4;
 ...
 * - session: enterGuildRaid() 결과 { sessionId, guildId, weekKey, bossCurrentHp, bossMaxHp, bossAtk, bossDef, remainingAttempts }
 * - guildName: 상단 배지에 표시할 길드명
 * - onSettled(result): 전투 종료(승/패 무관) 후 서버 응답 { newCurrentHp, bossMaxHp, clearedNow } 전달
 * - onExit(): 목록으로 돌아가기
 */
export default function GuildRaidBattle({ initialMonster, equipmentBonus, equippedSkills, equippedCostumes, session, guildName, onSettled, onExit }) {
  ...
  const [enemy, setEnemy] = useState(() => ({
    name: `길드 레이드 · ${guildName ?? '우리 길드'}의 수호자`,
    element: 'grass',
    spriteKey: 'grass_1',
  ...
  const [log, setLog] = useState('길드 레이드 보스가 그 모습을 드러냈다!');
  ...
  useEffect(() => {
    ...
    if (enemy.hp <= 0 || player.hp <= 0) {
      ...
      setResult(outcome);
      setSettling(true);
      setLog(outcome === 'win' ? '레이드 보스에게 강력한 일격을 꽂아넣었다!' : `${player.name}가 쓰러졌다... 입힌 피해는 그대로 기록돼요.`);
      reportGuildRaidDamage(session.sessionId, Math.round(damageDealtRef.current))
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (Date.now() < enemyStunnedUntil) {
        setLog('레이드 보스가 기절해서 움직이지 못한다!');
        return;
      }
      enemyAttackCountRef.current += 1;
      const isSkillAttack = enemyAttackCountRef.current % GUILD_RAID_SKILL_EVERY_N_ATTACKS === 0;
      ...
      const baseDamage = isSkillAttack ? GUILD_RAID_SKILL_DAMAGE : GUILD_RAID_NORMAL_DAMAGE;
      const finalDamage = Math.round(baseDamage * escalation);
      setLog(isSkillAttack ? '레이드 보스가 강력한 스킬을 사용했다!' : '레이드 보스의 포효!');
      damagePlayer(finalDamage);
    }, GUILD_RAID_ATTACK_INTERVAL);
    return () => clearInterval(timer);
  ...
  function useSkill(skill) {
    ...
    if (skill.type === 'damage') {
      ...
      setLog(`${player.name}의 ${skill.name}! 레이드 보스를 ${(stunMs / 1000).toFixed(1)}초간 기절시켰다!`);
  ...
  return (
    <div className={`battle-screen worldboss-screen ${shake ? 'shake' : ''}`}>
      <div className="stage-badge">
        🛡️ 길드 레이드 · 남은 도전 {session.remainingAttempts}회
      ...
      <div className="arena worldboss-arena">
        ...
        <div className="worldboss-giant-wrap">
          <span className="worldboss-icon">🛡️</span>
        </div>
      ...
      <div className="hud-row">
        ...
        <HpBar label={enemy.name} hp={enemy.hp} maxHp={enemy.maxHp} color="#5cb83c" formatNumbers />
      </div>
      ...
      {result ? (
        <div className="result-panel">
          <p className="result-text">
            {result === 'win' ? '레이드 보스에게 마지막 일격 성공!' : result === 'timeout' ? '⏱️ 제한시간 종료' : '퇴각...'}
          </p>
