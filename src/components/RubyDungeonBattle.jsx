 ...
 * - rubyBoss: rubyDungeon.js의 getRubyDungeonBoss() 결과
 * - onWin(grownBaseMonster): 승리 시 (루비 지급/세션 클레임은 상위에서 처리)
 * - onExit()
 */
export default function RubyDungeonBattle({ initialMonster, equipmentBonus, equippedSkills, equippedCostumes, jobSkillEnhancements, rubyBoss, onWin, onExit }) {
  ...
  const [enemy, setEnemy] = useState(() => ({ ...rubyBoss }));
  ...
  const [log, setLog] = useState(`${rubyBoss.name} 등장! 루비를 걸고 싸워보세요.`);
  ...
  useEffect(() => {
    ...
    if (enemy.hp <= 0) {
      ...
      const grownBase = applyExpGain(base, rubyBoss.expReward);
      setPlayer(withEquipment(grownBase, equipmentBonus));
      setLog(`${enemy.name} 처치! 루비 지급 처리 중...`);
      playVictorySound();
      onWin?.(grownBase);
    } else if (player.hp <= 0) {
      setResult('lose');
