...
**스킬 보유효과** (`getSkillPossessionBonus`/`sumSkillPossessionBonus`, `skillCatalog.js`): 장비처럼 스킬도 **보유만 하고 있으면(장착 여부 무관) 상시 공격력 보너스**를 줌. 등급별 기준치 노멀2/레어4/에픽8/전설16/신화32에 `1+(스킬레벨-1)*0.01`을 곱해서 스킬레벨이 오를수록도 같이 커짐. `App.jsx`가 `sumSkillPossessionBonus(userSkills)`로 보유 스킬 전체 합산치를 계산해서 장비 보너스(`equipmentBonus.atk`)에 더해 전투에 반영함 — 즉 실제 전투력에는 "장착 4보너스 + 장비 보유효과 + 스킬 보유효과"가 전부 누적됨. 스킬 뽑기 화면에 개별 스킬 카드마다, 그리고 전체 합계도 표시됨
...
### 5-6. 아이템 강화 (~~폐지됨, 014~~) / 합성 (`synthesize_equipment`, 026)
...
- **장비 합성(026, 028)**: 인벤토리 화면에서 아이템별로 "합성"(1회) / "일괄합성"(가능한 만큼 반복) 버튼 제공. **강화수치 10을 소모**해서 **같은 슬롯의 상위 등급 아이템 강화수치를 +1** 올림(`synthesize_equipment`/`synthesize_equipment_batch` RPC). 일괄합성은 서버에서 한번에 반복 처리(왕복 여러 번 안 함), 결과로 몇 회 합성됐는지(`times`)도 반환. 신화 등급은 더 합성 불가
  - ⚠️ 구현상 트레이드오프 설명: 원래 요청은 "장비 10개를 합성하면 1개의 상위등급"이었지만, 이 게임은 뽑기 중복이 별도 인벤토리 행(재고)이 아니라 `enhance_level` 누적으로만 반영되는 구조(`user_inventory`에 `unique(user_id, item_key)` 제약, 014/017)라 애초에 "같은 아이템 10개를 따로 보유"하는 개념 자체가 없음. 그래서 "강화수치 10 = 그동안 모은 재료 10개"로 치환해서 매핑함 — "합성할수록 보유효과도 반영"이라는 요구사항은 상위 등급 아이템의 `enhance_level`이 실제로 오르니까 자연히 충족됨(보유효과 공식이 `enhance_level`에 비례하므로 별도 로직 불필요)
...
- `synthesize_equipment(p_item_key)` RPC 신설 - 강화수치 10 소모 → 상위 등급 강화수치 +1 (5-6 참고)
...
- ⚠️ **보안 패치(027)**: 채팅은 RPC가 아니라 client가 테이블에 직접 INSERT하는 구조라, 닉네임 사칭은 트리거로 막혀있었지만(004) **전송 속도 제한이 없어서 봇으로 도배가 가능했음**. `chat_rate_limit_guard` 트리거로 **본인 기준 최소 2초 간격**을 강제하도록 수정

### 5-18. PvP (`PvP.jsx`, `PvPArena.jsx`, `PvPShop.jsx`, `pvp.js`, 023)

- **비동기 매칭 전투** — 실시간 대결이 아니라, 도전 버튼을 누르면 서버가 그 즉시 상대를 찾고 승패까지 한번에 판정함. 다만 **결과를 즉시 보여주지 않고**, `PvPBattleScene.jsx`가 서버가 이미 정한 결과(승/패, 내 전투력, 상대 전투력)를 약 2.7초짜리(6라운드×450ms) HP바 애니메이션으로 먼저 연출한 뒤에 결과 패널을 표시함(`PvPArena.jsx`가 `pendingBattle`/`lastResult` 두 단계 state로 분리 관리). 진짜 실시간 조작 대전은 아니고 "이미 정해진 결과를 그럴듯하게 재생"하는 방식
...
- ⚠️ **버그 수정 이력**: 전투 직후 화면의 "🏆 N승 · 💀 M패" 표시가 **다음 전체 프로필 재조회 전까지 안 바뀌는** 버그가 있었음 — `PvPArena.jsx`가 재화(`pvp_currency`)만 상위로 올려주고 승패 전적은 상위 `profile` state에 반영하는 경로가 아예 없었던 게 원인. `onCurrencyChange` 대신 `onBattleResolved(res)` 콜백으로 바꿔서, `App.jsx`가 `res.result`를 보고 `pvp_wins`/`pvp_losses`까지 함께 즉시 갱신하도록 수정
