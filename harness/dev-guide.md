...
- ⚠️ **로그인 전(anon) 화면에서 호출할 함수가 아니라면, 새로 만드는 SECURITY DEFINER 함수는 기본적으로 anon 실행이 막혀있다고 가정하지 말 것**(사용자 제보, migration 182) — PostgreSQL은 함수 생성 시 PUBLIC(anon 포함)에 자동으로 EXECUTE 권한을 줌. 함수 몸통에 `auth.uid() is null` 체크가 있어도 "실행을 시도할 권한 자체"는 별개로 열려있는 상태이므로, Supabase 린터가 "Public Can Execute SECURITY DEFINER Function"으로 지적함. 로그인 전 화면(`AuthScreen.jsx`)에서 실제로 호출하는 함수(`is_nickname_taken`, `find_masked_email_by_nickname`, `fetch_total_achievement_claims`)가 아니라면, 새 마이그레이션에 `revoke execute on function public.함수명(인자타입...) from public; grant execute on function public.함수명(인자타입...) to authenticated;`를 추가할 것
...
- **새 쿠폰을 발행할 때마다 [`coupon.md`](./coupon.md)의 표에 코드/보상/최대사용/만료일/발행 사유(migration 번호)를 추가할 것** — 쿠폰 목록의 단일 진실 공급원은 `coupon.md`이며, 다른 문서(`mailbox-and-coupons.md` 등)에는 개별 코드를 중복 기재하지 않는다
- **새 파일(컴포넌트/훅/유틸)을 만들기 전에는 반드시 [`architecture.md`](./architecture.md)를 먼저 읽고, 그 기준에 따라 어느 폴더(`components/atoms`·`molecules`·`organisms`·평면, `hooks/`, `lib/`)에 어떤 이름으로 만들지 판단할 것**(사용자 요청, 2026-08-01) — 폴더 구조를 바꾸는 리팩토링을 하면 `architecture.md`도 같은 커밋에서 갱신할 것
- **유저가 체감 가능한 기능 추가/변경/버그수정은 harness 문서와 별개로 `src/lib/patchNotes.js`(`PATCH_NOTES`)에도 반드시 기록할 것**(사용자 요청, 2026-08-01) — harness는 개발자용 기술 문서고, `PATCH_NOTES`는 게임 내 유저에게 보여주는 화면(`PatchNotes.jsx`)이라 별도로 챙겨야 함. 파일 하나에 이미 규칙이 적혀있으니 반드시 준수: (1) 날짜는 실제 배포일(오늘 날짜) 기준, (2) 같은 날짜에 여러 번 배포해도 그날 그룹의 `items` 배열 앞에 이어붙이기만 할 것(새 날짜 그룹 남발 금지), (3) 최신이 항상 `PATCH_NOTES[0]`. 내부 문서 정리(harness 갱신, 리팩터링, 점검 로그 등 유저가 체감할 수 없는 변경)는 패치노트에 넣지 않는다 — 유저 관점에서 "뭐가 달라졌는지"만 기록
