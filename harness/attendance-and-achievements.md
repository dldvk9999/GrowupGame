...
- ⚠️ **대량 확장(migration 152, 신규, 사용자 요청 — "총 업적 약 200개 가까이")**: 기존 55개에 143개를 추가해서 **총 198개**로 확장. **새 카테고리를 만들지 않고, 이미 있던 18개 stat(레벨/전직차수/스테이지클리어/뽑기횟수/PvP승수/복수전승수/누적골드/월드보스누적피해/장비종류수/유물보유수/유물최고강화/장비최고강화/코스튬수/전투력/던전깊이/추천인수/출석일수/무한의탑최고층)의 중간 단계를 촘촘하게 채우는 방식**으로 확장함 — 서버가 이미 계산해두고 있는 로컬 변수(`v_monster.level`, `v_pvp_wins` 등)를 그대로 재사용할 수 있어서 새로운 데이터 집계 로직을 추가할 필요가 없었고, 유저 입장에서도 "다음 목표가 너무 멀지 않은" 자연스러운 성장 곡선이 됨(예: 레벨 업적이 기존 10/30/60/100/140/180 6개 → 20/40/50/70/80/90/110/120/150/160/170/190/200/220/250/280/300/350/400/450/500까지 촘촘하게 채워짐)
- 생성 과정: Python 스크립트로 각 카테고리별 (target, 아이콘, 제목, 보상) 목록을 만들고, 클라이언트 카탈로그 라인과 서버 SQL CASE 분기를 동시에 생성해서 양쪽이 처음부터 100% 동기화되도록 함(수작업으로 143개를 양쪽에 각각 옮겨적으면서 실수할 위험을 원천 차단)
...
## 업적 검색 + 조건 팝업 (신규 콘텐츠, 사용자 요청)

업적이 늘어나면서 원하는 업적을 찾기 어려워져서 검색창을 추가함.

- **검색**: 제목(`title`)/설명(`desc`) 텍스트에 포함되는지로 실시간 필터링(대소문자 무관). 검색어가 있으면 매칭되는 업적이 하나도 없는 카테고리는 아예 숨기고, 매칭이 있는 카테고리는 접힘 상태와 무관하게 강제로 펼침(검색 결과가 접힌 카테고리 속에 숨어서 안 보이는 걸 방지) — 검색어를 지우면 원래의 접기/펼치기 상태(`manuallyToggledCategories`)로 자연 복귀
- **조건 팝업**: 각 업적 제목 옆 "ℹ️" 버튼을 누르면 `modal-backdrop` + `story-popup-card` 패턴(기존 스토리 팝업과 동일)의 작은 팝업이 뜸 — 아이콘/제목/설명/목표수치/보상골드/칭호(있으면)를 한 번에 보여줌. 순수 클라이언트, 서버 호출 없음(이미 `ACHIEVEMENT_CATALOG`에 있는 정보를 그대로 표시)
