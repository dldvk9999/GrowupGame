...
## 시즌 패스 (migration 178, 신규 콘텐츠, 사용자 요청 - 리텐션 강화)

매달 자동으로 리셋되는(`season_key = 'YYYY-MM'`, 한국시간 기준) 20단계 보상 트랙. 배틀패스형 리텐션 장치 — "오늘 안 하면 뒤처진다"는 압박을 자연스럽게 만드는 게 목적.

- **포인트 획득**: 새로운 콤보/타이머를 안 만들고, 이미 있는 두 클레임 액션에 포인트 지급을 얹음 — 출석체크(`claim_attendance`, 056) +20, 가이드미션 완료(`claim_mission_reward`, 081) +30. 둘 다 이미 각자의 안티치트(하루 1회/2초 게이트)가 있어서 시즌 패스만을 위한 새 방지장치가 필요 없었음
- **보상 수령**: `claim_season_tier(tier)` — 요구 포인트(`티어*150` 누적)와 보상액(골드 위주, 5단계마다 루비 소량 포함)은 전부 **서버 CASE문 안에서만 결정**함. 클라이언트가 금액을 파라미터로 넘기는 구조가 아님 — 넘기게 만들면 임의 금액 위조가 가능해지므로 설계 단계에서부터 배제함(클라이언트 `getSeasonTierInfo()`는 순수 표시/미리보기용이고, 서버 CASE문과 반드시 동기화해서 유지해야 함)
- `season_pass_progress` 테이블(유저당 시즌마다 1행) — `claimed_tiers`를 정수 배열로 저장해서 중복 수령 방지
- UI(`components/organisms/SeasonPassPanel.jsx`)는 출석체크 모달에서 "🎫 시즌 패스 보러가기" 버튼으로 진입 — 헤더에 새 버튼을 또 추가하는 대신 이미 있는 "출석체크" 진입점을 재사용(모바일 헤더가 이미 붐빔, `guild.md`의 "친구/길드" 헤더 개선 때와 동일한 절제 원칙)
