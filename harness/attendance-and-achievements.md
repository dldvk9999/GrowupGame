...
게임 화면(전투/스테이지 등) 상단, 탭 네비게이션 바로 위에 뜨는 작은 위젯. 출석체크·무료뽑기·가이드미션 3가지의 오늘자 완료 여부를 한눈에 보여주고, 미완료 항목을 클릭하면 바로 해당 화면(출석체크 모달/상점)으로 이동함. **3개 다 완료하면 위젯 자체가 사라져서** 화면을 불필요하게 가리지 않음.

- 새 서버 호출 없음 — `App.jsx`가 이미 로그인 시점에 로드해둔 `attendanceState`/`freeDrawUsedToday`/`missionCompleted`를 그대로 조합만 함
