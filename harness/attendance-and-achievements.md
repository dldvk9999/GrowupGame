...
**2차 해결**: [`ui-and-ux.md`](./ui-and-ux.md)의 "탭 전환/백그라운드 복귀 시 마지막 탭이 유지 안 되던 버그" 수정(`sessionRef`로 "같은 유저의 세션 재확인"을 판별해서 리셋 로직 전체를 건너뜀)이 이 문제도 함께 해결함 — 더 이상 세션 갱신만으로는 `hasClaimedMissionToday`가 리셋되지 않으므로, 별도 코드 변경 없이 자동으로 고쳐짐. **교훈**: 겉보기엔 다른 두 버그 제보(탭 리셋, 미션 재완료 표시)가 사실은 같은 근본 원인(세션 갱신 시 무차별 초기화)에서 나온 증상이었음 — 비슷한 시점에 리셋되는 여러 state가 있다면 그 초기화 로직 자체를 먼저 의심해볼 필요가 있음.

**⚠️ 2차 해결로도 재발함(사용자 3차 제보) — "페이지 새로고침"은 애초에 다른 종류의 이벤트였음**: `sessionRef` 판별은 "같은 앱 인스턴스 안에서 세션만 갱신되는 경우"(백그라운드 복귀, 토큰 리프레시)만 커버했는데, **진짜 페이지 새로고침(F5)은 React 앱 자체가 처음부터 완전히 다시 실행**되므로 `sessionRef.current`도 초기값(`null`)으로 시작함 — 즉 새로고침은 `isSameUserRefresh`가 항상 `false`가 되어 "최초 로그인"과 동일하게 취급되고, `hasClaimedMissionToday`도 매번 `false`로 리셋됨. 애초에 `hasClaimedMissionToday`가 **순수 React 메모리 state였고 어디에도 영속 저장되지 않았던 것**이 근본 원인 — 새로고침하면 메모리는 통째로 날아가니 당연한 결과였음.

**최종 해결**: `lib/missionClaimPersist.js`를 새로 만들어서 `hasClaimedMissionToday`를 **localStorage에 유저ID+오늘날짜로 영속화**함(이미 확립된 `dailyQuote.js`/`loginStreak.js` 패턴과 동일). 로그인 시점에 `hasClaimedMissionTodayPersisted(userId)`로 초기값을 복원하고(무조건 `false`로 리셋하던 걸 대체), 미션 수령 성공 시 `markMissionClaimedToday(userId)`로 기록함. 헤드리스 브라우저로 4가지 시나리오(최초/완료후재확인=새로고침시뮬/다른유저분리/날짜경과시리셋) 전부 정확히 동작함을 검증. **교훈**: React state만으로는 "오늘 있었던 일"을 정확히 표현할 수 없음 — 새로고침에도 살아남아야 하는 값은 처음부터 로컬 저장소 기반으로 설계해야 함(이번에 세 번의 시행착오 끝에 도달한 결론).
...
업적이 44개까지 늘어나면서 한 화면에 다 펼쳐두면 스크롤이 길어지는 문제가 있어서, 카테고리별로 접기/펼치기 가능하게 함. **기본값**: 그 카테고리 업적을 이미 전부 달성했으면 접혀서 시작(더 볼 필요 없는 완료 카테고리를 자동으로 숨김), 아직 남은 게 있으면 펼쳐서 시작. 사용자가 헤더를 한 번이라도 클릭하면 그 이후로는 자동판정 대신 사용자의 마지막 선택이 우선 적용됨(`manuallyToggledCategories` state). 순수 클라이언트 UI 상태, 서버 호출 없음.

## 출석체크 모달에 마일스톤 진행률 안내 추가 (신규 콘텐츠)

출석체크 모달 상단에 "🏅 누적 출석 N회 — 다음 업적까지 M회 남음"을 표시해서, 출석 마일스톤 업적(7/30/100/200/365)의 존재를 자연스럽게 알려줌. 이미 있는 `attendanceState.total_claim_count`만 계산, 새 서버 호출 없음.
