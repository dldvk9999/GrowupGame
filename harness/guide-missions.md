...
`claim_mission_reward`가 다음 `spend_gold` 미션을 배정할 때, **스킬 뽑기레벨**(`profiles.total_skill_draws` 기준)과 **4개 장비 슬롯 뽑기레벨 평균**(`equipment_gacha_progress` 기준)을 합쳐 "종합 뽑기레벨"(1~**50**, 045에서 20→50으로 확장)을 구하고, 이 레벨에 따라 목표치를 **10만 골드 단위**로 계단식 상향함:
...
보상도 목표치의 1%로 같이 스케일링됨(예: 50만 목표면 보상 5000골드). 뽑기를 거의 안 한 신규 유저는 최소 10만부터 시작하고, 뽑기레벨이 최대(50)에 가까운 유저는 50만까지 오름.
...
클라이언트가 진행도를 부풀려 보낼 수는 있지만(예: `bumpMission('spend_gold', 1000)`을 반복 호출), 미션 보상 자체가 소액(일반 미션 600~1200골드 수준, `spend_gold`만 043부터 뽑기레벨에 따라 최대 5000골드까지 커짐, 온보딩 미션만 예외적으로 더 큼)이라 리스크 대비 실효성이 낮고, 온보딩 미션(가장 보상이 큰 것들)은 서버가 실제 게임 상태로 재검증하므로 조작 불가능함. 클레임 자체도 2초 쿨다운(위 참고)으로 막혀있어서, `spend_gold` 보상이 커졌어도 시간당 파밍 가능한 절대량은 제한적임.
...
`claim_mission_reward`의 2초 쿨다운이 `mission_state.updated_at` 기준이었는데, `increment_mission_progress`도 진행도를 올릴 때마다 `updated_at`을 같이 갱신해서 — **미션을 막 완료시킨 마지막 진행도 증가 직후 완료 버튼을 눌러도 "너무 빠릅니다" 오탐이 뜨는 문제**가 있었음.
...
⚠️ **081에서 20초 → 2초로 단축**(사용자 피드백) — "미션을 완료하자마자 바로 다음 미션으로 넘어가고 싶은데 20초씩 기다려야 하는 게 답답하다"는 요청. 매크로 스팸 방지라는 원래 목적은 2초로도 어느 정도 유지되고(완전히 없애는 것보다는 안전), 진행도를 실제로 채워야 완료 판정이 나는 구조(서버가 `progress >= target`을 재검증)라 클레임 간격을 줄인다고 파밍 속도가 크게 늘지는 않음 — 어차피 미션 자체를 다시 완료해야 다음 클레임이 가능하므로.
