...
- **실시간 접속자 수 표시**(Presence) — `useLobbyChat`이 같은 `lobby-chat` 채널에 Supabase Realtime **Presence**로 자기 자신을 `track()`하고, `presenceState()`의 키 개수를 "지금 로비에 접속 중인 인원"으로 헤더에 "🟢 N명 접속 중"으로 보여줌. 다른 유저와의 상호작용을 눈에 보이게 해서 "사람이 있다"는 사회적 실재감을 주는 장치(같은 유저가 탭을 여러 개 열면 각각 별도로 카운트됨 — 정확한 순유저 수가 아니라 "지금 열려있는 연결 수" 근사치)
...
기존엔 전투력 랭킹(로비)/업적 랭킹(설정>업적 화면 토글)/무한의 탑 랭킹(던전>탑 화면)이 서로 다른 화면에 흩어져 있었음. `Leaderboard.jsx`(로비의 "🏆 랭킹" 서브탭) 상단에 종류 선택 탭(⚔️전투력/🏆업적/🗼무한의탑)을 추가해서, 한 화면에서 세 랭킹을 전부 스위칭하며 볼 수 있게 함.
...
랭킹 화면에 진입할 때마다, 저번에 봤을 때 순위 대비 얼마나 오르내렸는지 "▲5" / "▼3" 배지로 보여줌. 5개 종류(전투력/업적/탑/친구추천/골드) 전부 각각 독립적으로 추적됨(localStorage에 종류별 키로 저장). **순위 판정 자체는 항상 서버가 계산한 최신 값을 그대로 쓰고, 이 모듈은 순수하게 "직전에 로컬에 저장해둔 값과의 차이"만 계산**하므로 실제 순위 정확성에는 전혀 관여하지 않는 순수 재미 요소. 헤드리스 브라우저로 연속된 순위 변화 시나리오(첫 조회/상승/하락/순위권 이탈)를 실제 실행해서 계산 정확성 검증 완료, 큰 순위 숫자(1234위) 조합 모바일 넘침도 재검증.
