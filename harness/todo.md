...
- **용의 버프(20배)가 PvP/랭킹 서버 계산에 미반영**(코드 재검토 중 확인, [`world-boss.md`](./world-boss.md) 참고) — 스테이지/던전 전투에서만 적용되고 PvP/랭킹은 그대로임. 버그인지 의도된 공정성 설계인지 불확실해서, 밸런스에 큰 영향을 줄 수 있는 변경이라 사용자 확인 없이 손대지 않음.
