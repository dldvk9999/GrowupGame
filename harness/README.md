...
마지막 정리: migration 180까지 반영. 자세한 변경 이력은 각 문서 하단과 `security.md`의 정기점검 로그 참고.
...
| [`sealed-dungeon.md`](./sealed-dungeon.md) | 봉인된 던전 - 골드/경험치 없이 파편만 주는 장기목표형 던전(재화 인플레 방지 설계) |
...
- **컨셉**: 3속성(불/물/풀) 스타터 몬스터를 계약해 100개 챕터×10스테이지를 돌며 육성/전직/장비강화. PvP와 로비 채팅으로 다른 유저와 상호작용
...
│  │  │  └─ TowerPanel.jsx, ExpeditionPanel.jsx, SeasonPassPanel.jsx
...
│  │  ├─ SealedDungeonBattle.jsx   # 봉인된 던전 전투 화면(경험치 없음, 파편만 지급) - sealed-dungeon.md
...
│     ├─ seasonPass.js              # 시즌 패스 RPC 연동 - attendance-and-achievements.md
...
│     ├─ guild.js                  # 길드 시스템 RPC 연동 - guild.md
...
- 로그인 시 `getActiveMonster()`로 활성 몬스터, `fetchClearedStageIds()`로 클리어 기록을 불러와 **최고 클리어+1 스테이지부터 자동으로 이어서 시작**("로그인하면 저장된 지점부터 재개" 요건 충족)
