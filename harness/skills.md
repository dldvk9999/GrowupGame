...
상점 화면 상단에 "🎁 오늘의 무료뽑기" 버튼이 있어서, **하루 1회 골드 소비 없이** 현재 선택된 탭을 공짜로 뽑을 수 있음. 출석체크와는 별개 장치(로그인 자체에 대한 보상이 아니라 "가챠를 무료로 한 번 더 당길 수 있게" 해주는 것) — 가챠 장르에서 흔한 재방문 유도 패턴.
...
무료로 뽑아도 `total_skill_draws`/`equipment_gacha_progress.total_draws`는 정상적으로 증가하므로 뽑기레벨 성장이나 업적(`gacha_100` 등)에도 그대로 반영됨.

### 5종(무기/방어구/장갑/신발/스킬) 독립 분리 (migration 101, 사용자 요청)

원래는 "스킬 또는 장비(슬롯 선택) 중 1종만 하루 1회"였는데, 이제 **무기·방어구·장갑·신발·스킬 5종 전부 각각 독립적으로 하루 1회씩** 무료로 뽑을 수 있음. `daily_free_draw_state`를 `(user_id, draw_type)` 복합키로 재구성(`draw_type`: weapon/armor/gloves/shoes/skill 5가지 체크 제약). `claim_daily_free_draw(p_type)`의 `p_type`이 곧 슬롯/스킬 구분값을 겸하도록 단순화해서 기존 `p_slot` 파라미터를 제거함(함수 시그니처 변경이라 `DROP FUNCTION` 선행).

- 정책 자체가 "하루 1개"에서 "하루 5개"로 바뀌는 시점이라, 기존 상태 테이블을 그대로 마이그레이션하지 않고 통째로 재생성함 — 배포 직후엔 유저 전원이 5종을 전부 새로 받을 수 있게 되는 것뿐이라 게임 경제 영향은 미미함(무료뽑기 자체가 저비용 항목)
- 클라이언트(`dailyFreeDraw.js`)도 `hasUsedFreeDrawToday(단일 boolean)` → `buildFreeDrawUsedMap(5종 객체)`로 재작성, `Shop.jsx`가 현재 선택된 탭 기준으로 그 탭의 무료뽑기 상태만 표시하고, 5개 탭 버튼 각각에도 "아직 무료뽑기 가능" 점(dot)을 표시해서 어느 슬롯이 남았는지 한눈에 보이게 함
- 헤드리스 시뮬레이션으로 부분 사용(무기+스킬만 받은 상태)/신규유저(빈 배열)/조회실패(null) 세 케이스 모두 정확히 판정됨을 검증
...
스킬 편성 화면에 "📚 스킬 컬렉션 N/50" 진행률 바 추가 — 코스튬 컬렉션(077)과 동일한 CSS 패턴 재사용. "만능 마스터" 업적(092, 50종 전부 보유)과 자연스럽게 연동되어 컬렉션 진행 상황을 시각적으로 보여줌. 이미 있는 `ownedMap`(보유 스킬 조회 결과)의 크기만 계산, 새 서버 호출 없음.
