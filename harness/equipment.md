...
## 장비 컬렉션북 (migration 121, 신규 콘텐츠, 사용자 요청)

`user_inventory`는 판매/삭제 경로가 없어서(위 "인벤토리 화면" 참고) 한번 얻은 `item_key`는 영구히 남음 — 즉 "역대 보유한 고유 아이템 종류 수"가 새 테이블 없이도 `count(distinct item_key)`로 이미 추적되고 있었음. 이 값을 기준으로 업적 2종만 추가:

- **📦 수집가의 눈**: 고유 장비 10종 보유(20종 중 절반), 4000골드
- **🏛️ 장비 명예의 전당**: 4슬롯×5등급 20종을 전부 한 번씩 보유, 20000골드
