...
캐릭터 카드 하단에 "🎯 다음 목표" 칩 리스트 추가 — PvP 다음 티어까지 남은 승수, 무한의 탑 다음 10층 마일스톤까지 남은 층수, 다음 출석 마일스톤(7/30/100/200)까지 남은 횟수, **코스튬 컬렉션 완성까지 남은 종수(20종 기준)**를 한 곳에 모아서 보여줌. 여러 시스템에 흩어진 "다음 목표"를 순수 클라이언트 계산으로 통합(서버 호출 없음, 이미 로드된 데이터만 조합) — `pvpTier.js`의 `getWinsToNextTier` 등 기존 헬퍼를 재사용. 4개 칩이 동시에 표시되는 경우도 모바일 실측 검증 완료(넘침 없음).
