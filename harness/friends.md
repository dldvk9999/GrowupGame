...
- ⚠️ **버그 수정(사용자 제보)**: "UID 형식 검증 없음"이라고 위에 적었지만 실제로는 문제가 있었음 — `send_friend_request`의 `p_target_id`가 `uuid` 타입이라, uuid 형식이 아닌 문자열을 넣으면 내 함수 본문에 진입하기도 전에 PostgREST/Postgres가 영어 에러("invalid input syntax for type uuid")를 그대로 던졌음. `friends.js`에 uuid 정규식 검증을 추가해서 형식이 안 맞으면 RPC 호출 전에 한글 메시지("올바른 UID 형식이 아니에요")로 먼저 막음
- ⚠️ **버그 수정(사용자 제보)**: 최초 배포 시 `.friends-add-form input`에 스타일이 전혀 없어서 브라우저 기본 흰색 인풋으로 보여 다크 테마와 이질감이 컸음. 로그인 화면(`.field input`)과 동일한 다크 배경/테두리/포커스 스타일을 적용해서 통일.
