...
- **`src/hooks/useExpGoldDungeon.js`** — 경험치/골드 던전(순차 스테이지형) 상태 5개 + 핸들러 2개(콤보보너스/정예몬스터/행운/골든타임/요일보너스 5가지 토스트 분기 포함, `setActiveMonster`/`setProfile` 외부 전달)

`App.jsx` 1,494줄 → 1,186줄. 아직 갈 길이 멀지만(PvP/우편함/업적/친구/뽑기 등 훨씬 많은 기능이 남음), 매번 "외부에서 이 state/setter를 또 어디서 쓰고 있는지"(특히 초기 로드 `Promise.all`과 로그아웃 리셋 블록)를 `grep`으로 먼저 확인하고 나서만 추출하는 원칙을 지키고 있어서 지금까지 리스크 없이 진행 중.
